...
function screenr_scripts() {
	...
	wp_enqueue_style('style', '/wp-content/themes/screenr-child/assets/css/style.css');

	wp_enqueue_script('script', '/wp-content/themes/screenr-child/assets/js/script.js');
...
function list_category( $atts ) {
    ...
    $return = "";
        ...
        $catPost = get_posts($atts);

        $return .= '<div>';

            foreach ($catPost as $post) : setup_postdata($post);

            $return .= '<h1>' . get_field('designation', $post) . '</h1>';
			$return .= '<img src="' . get_field('image', $post) . '" >';
				$return .= '<ul>';
                        $return .= '<li>' . get_field('prix', $post) . '</li>';
                        $return .= '<li>' . get_the_content() . '</li>';
                        $return .= '<a href="' . get_permalink($post) . '">Voir l\'article.</a>';
                    $return .= '</ul>';
            endforeach;
        $return .= '</div>';
    return $return;
}
add_shortcode( 'list_category_shortcode', 'list_category' );